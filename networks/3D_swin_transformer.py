...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
                 dim,           # dim = C
                 depth,         # 跟 feature 的 D 不同，是指的 block 深度，也就是这个 module 会有多少个 W-MSA/SW-MSA 摞在一起 (两种SA交替出现)
...
class PatchEmbed3D(nn.Module):
    ...
    Args:
        ...
        in_chans (int): Number of input video channels. Default: 155.
        embed_dim (int): Number of linear projection output channels. Default: 96.
        norm_layer (nn.Module, optional): Normalization layer. Default: None
    """
    def __init__(self, patch_size=(2,4,4), in_chans=155, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # D, H, W = patch_size[0], patch_size[1], patch_size[2], 防止除不开先padding
